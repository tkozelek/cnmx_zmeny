...
|---|---|---|---|---|
| `user:{id}:approved-teams` | `App\Models\User::approvedTeams()` | `Collection<Team>` of every team the user is an approved (non-pending) member of | forever | `App\Models\User::forgetApprovedTeamsCache()`, called from `App\Livewire\UsersDataTable::accept()` and `::deny()` — the only two places an *existing* user's `team_user.approved_at` changes. Brand-new memberships (`Admin\UserController::store`, `Auth\RegisterController`) never need invalidation: a new user id has no cache entry yet. |
