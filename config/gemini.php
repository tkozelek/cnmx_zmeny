...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Which model AiRozpisSuggestionService asks. Flash by default: the fairness
    | ranking is computed before the call, so the model is pairing two pre-sorted
    | lists against a set of hard constraints, not reasoning from scratch.
    |
    | Not part of the package's own config — added here so the model sits next to
    | the key it is used with rather than in a second config file.
    */
