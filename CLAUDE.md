...
**Other domain pieces**: `Holiday`/`HolidayService` (unavailability marking — planned rename to `UnavailabilityService` with deadline enforcement), `File`/`FileService`/`FileUploadController` (per-week uploads, reused via Excel export), `Bug`/`BugReportController` (in-app bug reporting — planned for removal). `Shift`/`Rate` (hours + pay-rate tracking / "evidencia hodín") was removed entirely — no models, no tables (dropped by `2026_09_12_130000_drop_shifts_and_rates_tables.php`), no views.
