{{-- How the builder works and what every setting does, in the order a manager needs it.
     ...
     A native <dialog> rather than <x-modal>: that component routes opening through an Alpine
     custom event and an x-teleport, and this panel is plain read-only text with no Livewire state
     in it. showModal() gives the backdrop, Esc, the focus trap and inertness for free, and cannot
     be broken by whatever else on the page Alpine is or is not managing. --}}
@php
    ...
    <form method="dialog" class="flex items-start justify-between gap-4 border-b border-neutral-800 px-6 py-4">
        <div>
            <h2 class="flex items-center gap-2 text-lg font-bold text-white">
                ...
                Rozpis - návod a nastavenia
            </h2>
            <p class="mt-1 text-xs text-neutral-400">Ako sa rozpis zostavuje a čo sa kde dá nastaviť.</p>
        </div>
        ...
        <div class="rounded-lg border border-neutral-800 bg-neutral-950/50 px-4 py-3">
            ...
            <ul class="mt-2.5 flex flex-col gap-1.5 text-xs leading-relaxed text-neutral-400">
                ...
                <li class="flex gap-2">
                    ...
                    <span><strong class="text-neutral-200">Poradie spravodlivosti</strong> - medzi dvoma rovnako
        ...
        <div class="flex flex-col gap-4">
            <section class="rounded-lg border border-neutral-800 bg-neutral-950/50 px-4 py-3">
                ...
                <dl class="mt-2 grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
                    @foreach([
                        ...
                        ['Obdobie hodnotenia', $team->fairnessWindowWeeks().' týž.', 'Ako ďaleko do minulosti sa počítajú odpracované dni pre poradie spravodlivosti.'],
                    ] as [$label, $value, $note])
                        <div>
            ...
            <section class="rounded-lg border border-neutral-800 bg-neutral-950/50 px-4 py-3">
                ...
                <p class="mt-1.5 text-[0.7rem] leading-relaxed text-neutral-400">
                    ...
                    <strong class="text-amber-400">neobľúbený</strong> sa označí len deň s váhou
                    <strong>nad priemerom vášho týždňa</strong> (teraz {{ number_format($averageWeight, 2) }}).
                    Len v takom dni sa nezaradení zoradia podľa poradia spravodlivosti a zobrazí sa pri nich číslo.
                    Ak majú všetky dni rovnakú váhu, neoznačí sa žiadny.
                </p>
